...
const EditPost = ({
  ...
  const [newImagesFiles] = useState<File[]>([]);
    ...
    const fetchPost = async () => {
      try {
        ...
        if (data.estimatedTime) {
          ...
          const timeUnitToUse = data.estimatedTime
            .toLowerCase()
            .includes("hour")
            ? "hours"
            : "minutes";
        ...
        if (data.postMaterials && Array.isArray(data.postMaterials)) {
          ...
          const formattedMaterials = data.postMaterials.map(
            (material: any) => ({
              materialId: material.material?.id || material.materialId,
              quantityRequired:
                material.quantity || material.quantityRequired || 1,
  ...
  const allVideos: FileOrUrl[] =
    newVideosFiles.length > 0 ? newVideosFiles : videos;
      ...
      <form onSubmit={handleSubmit}>
        <PostFormFields
          ...
          onDifficultyChange={(value) =>
            updateField("postDifficulty", value as PostDifficulty)
          }
          onTypeChange={(value) => {
